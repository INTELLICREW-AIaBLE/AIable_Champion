...
const SEARCH_DATA: SearchItem[] = [
  ...
  { id: 'validator', category: 'tool', label: 'Ethics Guardrail', description: 'Phát hiện ảo giác & Thẻ tư duy phản biện', href: '/validator', icon: ShieldCheck },
...
export function AppNavbar() {
  ...
  useEffect(() => {
    ...
    const handleNewNotif = (e: any) => setNotifications(prev => [e.detail, ...prev].slice(0, 50));
    const handleReadNotif = () => setNotifications(getNotifications());

    window.addEventListener('storage', handleLangChange);
    window.addEventListener('app_lang_changed', handleLangChange);
    window.addEventListener('aiable_notification', handleNewNotif);
    window.addEventListener('aiable_notifications_read', handleReadNotif);
    
    return () => {
      window.removeEventListener('storage', handleLangChange);
      window.removeEventListener('app_lang_changed', handleLangChange);
      window.removeEventListener('aiable_notification', handleNewNotif);
      window.removeEventListener('aiable_notifications_read', handleReadNotif);
    };
  }, []);
  ...
  useEffect(() => {
    ...
    const handleGlobalKeyDown = (e: KeyboardEvent) => {
      ...
      // Do not trigger global shortcuts if typing in an input or textarea
      ...
      if (now - lastKeyTime > 1000) {
        lastKey = ''; // Reset if more than 1s passes
      }

      if (key === 'g') {
        lastKey = 'g';
        lastKeyTime = now;
      } else if (lastKey === 'g' && key === 'h') {
        router.push('/home');
        lastKey = '';
      } else if (lastKey === 'g' && key === 'p') {
        router.push('/profile');
        lastKey = '';
      } else if (lastKey === 'g' && key === 's') {
        ...
        <div className="flex items-center gap-2.5 shrink-0">
          ...
          <div className="relative" ref={notifRef}>
            <button
              ...
              className="p-2 text-slate-400 hover:text-violet-600 hover:bg-violet-50 rounded-full transition relative focus:outline-none focus:ring-2 focus:ring-violet-400"
            >
            ...
            {notifOpen && (
              <div className="absolute right-0 mt-2 w-72 rounded-2xl border border-slate-100 bg-white shadow-xl shadow-slate-200/60 py-2 z-50">
                <div className="px-4 py-2 border-b border-slate-100 flex justify-between items-center">
                  <span className="text-sm font-bold text-slate-800">Thông báo</span>
                </div>
                <div className="max-h-72 overflow-y-auto">
                  {notifications.length === 0 ? (
                    <div className="py-8 text-center text-sm text-slate-400">Không có thông báo nào</div>
                  ) : (
                    notifications.slice(0, 8).map(n => (
                      <div key={n.id} className="px-4 py-3 border-b border-slate-50 last:border-0 hover:bg-slate-50 transition cursor-default">
                        <div className="flex items-start gap-3">
                          <div className={`w-2 h-2 rounded-full mt-1.5 shrink-0 ${n.read ? 'bg-slate-200' : 'bg-violet-500'}`} />
                          <div>
                ...
                {notifications.length > 0 && (
                  <div className="px-2 pt-2 border-t border-slate-100 mt-1">
                    <Link href="/notifications" onClick={() => setNotifOpen(false)} className="block w-full text-center py-2 text-xs font-semibold text-violet-600 hover:bg-violet-50 rounded-xl transition">
                      Xem tất cả
                    </Link>
                  </div>
                )}
          ...
          <div className="relative" ref={avatarRef}>
            <button
              ...
              className="flex items-center gap-1.5 rounded-full focus:outline-none focus:ring-2 focus:ring-violet-400 focus:ring-offset-1 group relative"
              aria-haspopup="true"
            ...
            {avatarOpen && (
              <div className="absolute right-0 mt-2 w-56 rounded-2xl border border-slate-100 bg-white shadow-xl shadow-slate-200/60 py-2 z-50">
                ...
                {menuItems.filter(i => i.label !== text.notifications).map(({ icon: Icon, label, href, dividerAfter }) => (
                  <div key={href}>
                    <Link
                      href={href}
